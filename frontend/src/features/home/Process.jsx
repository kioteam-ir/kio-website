  ...
  return (
    <Container as="section" id="process" dir="rtl" className="py-20">
      ...
      <div className="relative mt-16 grid grid-cols-1 gap-8 md:grid-cols-5">
        <div className="absolute inset-x-0 top-6 hidden h-px bg-slate-800 md:block" aria-hidden="true" />
        {PROCESS_STEPS.map((item) => (
          <div key={item.step} className="relative">
            <div className="relative z-10 mb-5 flex h-12 w-12 items-center justify-center rounded-full border border-amber-400/40 bg-slate-950 font-mono text-sm text-amber-400">
              {item.step}
            </div>
