    ...
    <Container as="section" dir="rtl" className="py-16">
      <CornerFrame className="grid gap-10 rounded-lg border border-dashed border-slate-800 bg-slate-900/30 p-6 lg:grid-cols-2 lg:p-10">
        <div className="overflow-hidden rounded-md border border-slate-800">
          <img src={SPOTLIGHT.image} alt={SPOTLIGHT.title} className="h-72 w-full object-cover lg:h-full" />
        </div>
        ...
        <div className="flex flex-col justify-center">
          ...
          <h3 className="text-2xl font-bold text-white sm:text-3xl">{SPOTLIGHT.title}</h3>
          <p className="mt-4 text-sm leading-7 text-slate-400">{SPOTLIGHT.description}</p>

          <div className="mt-8 grid grid-cols-3 gap-4 border-y border-slate-800 py-6">
            {SPOTLIGHT.metrics.map((metric) => (
              <div key={metric.label} className="text-center">
                <div className="grad-brand-text font-mono text-xl font-bold sm:text-2xl">{metric.value}</div>
                <div className="mt-1 text-[11px] text-slate-500 sm:text-xs">{metric.label}</div>
              </div>
            ))}
          </div>
          ...
          <blockquote className="mt-6 border-e-2 border-brand-crimson-400/40 pe-4 text-sm italic leading-7 text-slate-300">
            «{SPOTLIGHT.quote}»
            <footer className="mt-2 font-mono text-xs not-italic text-slate-500">{SPOTLIGHT.quoteAuthor}</footer>
          </blockquote>
