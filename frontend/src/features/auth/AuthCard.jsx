    ...
    <div dir="rtl" className="relative flex min-h-screen items-center justify-center bg-slate-950 px-4 pt-24">
      ...
      <CornerFrame className="relative w-full max-w-md rounded-lg border border-dashed border-slate-700 bg-slate-900/70 p-6 shadow-2xl md:p-8">
        <div className="space-y-3 text-center">
          <span className="font-mono text-xs tracking-widest text-amber-400/80">KIO / ACCESS</span>
          <h2 className="text-xl font-bold text-white">{title}</h2>
          <Link to={subtitleTo} className="inline-block text-sm text-teal-300 transition-colors hover:text-teal-200">
            {subtitle}
          </Link>
        </div>
        {children}
